    ...
    HoprNodeSafeMigrationEvents
{
    ...
    function test_MigrateNodeSafeWithFailedERC1820Registry() public mockTokenChannel {
        uint256 callerPrivateKey = 0xCAFE;
        address caller = vm.addr(callerPrivateKey);
        address[] memory admins = new address[](1);
        admins[0] = caller;
        ...
        // mock a failed `setImplementer` call in the ERC1820 registry during the safe migration
        vm.mockCall(
