...
contract HoprNodeStakeFactoryTest is
    Test,
    ...
    HoprNodeStakeFactoryEvents
{
    ...
    bytes4 public deploySafeAndModuleSelector =
        bytes4(keccak256("_deploySafeAndModule(uint256,bytes32,address,address,uint256,address[])"));
    bytes4 public deploySafeAndModuleAndIncludeNodesSelector =
        bytes4(keccak256("_deploySafeAndModuleAndIncludeNodes(uint256,bytes32,address,address,uint256,address[])"));
    ...
    function testRevert_CloneSafeAndModuleWithFewOwner() public mockTokenChannel {
        ...
        (module, safe) = factory.clone(nonce, DEFAULT_TARGET, admins);
        vm.clearMockedCalls();
    }
    ...
    function test_CloneSafeAndModule() public mockTokenChannel {
        ...
        (address expectedSafeAddress, address expectedModuleAddress) =
            _helperPredictSafeAndModule(admins, caller, nonce);
    ...
    function test_OneClickDeploySafeWithFundTokenAndIncludeNodes() public mockTokenChannel {
        ...
        // prepare userData
        bytes memory userData =
            abi.encode(factory.DEPLOYSAFEANDMODULEANDINCLUDENODES_IDENTIFIER(), nonce, DEFAULT_TARGET, admins);
        ...
        for (uint256 i = 0; i < admins.length; i++) {
            (bool success, bytes memory retdata) =
                expectedModuleAddress.call(abi.encodeWithSignature("isNode(address)", admins[i]));
    ...
    function test_CloneMultipleSafesAndModules() public mockTokenChannel {
        ...
        (address module0, address payable safe0) = factory.clone(nonce0, DEFAULT_TARGET, admins0);
        ...
        (address module1, address payable safe1) = factory.clone(nonce1, DEFAULT_TARGET, admins1);
    ...
    function testRevert_SafeSetupReuseNonce() public mockTokenChannel {
        ...
        factory.clone(nonce, DEFAULT_TARGET, admins);
    ...
    function test_FallbackHandler() public mockTokenChannel {
        ...
        address implementer = IERC1820Registry(ERC1820_REGISTRY_ADDRESS)
            .getInterfaceImplementer(safe, keccak256("ERC777TokensRecipient"));
        ...
        hoprToken.mint(safe, 100_000, "", "");

        assertEq(hoprToken.balanceOf(safe), 100_000, "safe should have 100000 tokens");
        vm.clearMockedCalls();
    }
    ...
    function _helperDeployASafe(address admin0) private returns (address safeProxyAddr) {
        address[] memory admins = new address[](1);
        (address safeAddress, address safeProxyFactoryAddress, address compatibilityFallbackHandlerAddress,) =
        ...
        return address(SafeProxyFactory(safeProxyFactoryAddress).createProxyWithNonce(safeAddress, safeInitializer, 0));
    }
